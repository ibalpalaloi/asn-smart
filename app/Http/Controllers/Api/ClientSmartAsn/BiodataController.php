...
class BiodataController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if ($biodata){
            ...
            $biodata = DB::table('biodata_asn')->select('biodata_asn.*', 'asn.id as id_asn')->join('asn', 'asn.biodata_asn_id', '=', 'biodata_asn.id')->join('users', 'users.nip', '=', 'asn.nip')->where('users.id', $user->id)->first();
            $status = "new";
            $kecamatan = null;
            $jabatan_asn = null;
            $sub_bagian_selected = "";
